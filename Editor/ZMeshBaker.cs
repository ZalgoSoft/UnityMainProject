...
namespace ZMeshBaker
{
    public class ZMeshBaker : EditorWindow
    {
        enum Resolving
        {
            Not,
        ...
        private static bool FillDataAndCheckResolving(Dictionary<Material, List<Mesh>> meshesWithMaterials, List<Mesh> meshesWithoutMaterials)
        {
            ...
            foreach (GameObject selected in Selection.gameObjects)
            {
                ...
                foreach (var skinnedMeshRenderer in skinnedMeshRenderers)
                {
                    ...
                    Mesh mesh = ZBonesBaker.GetMeshFromSkinnedMeshRenderer(skinnedMeshRenderer);
            ...
            List<Vector2> uvs, List<int> triangles, Material material)
        {
            ...
            newMesh.name = "BakedMesh" + Guid.NewGuid().ToString("n").Substring(0, 4);
